...
const initDatabase = async () => {
  ...
  try {
    ...
    connection = await mysql.createConnection({
      ...
      password: process.env.DB_PASSWORD || 'Raj27'
    });
    ...
    await connection.execute('CREATE DATABASE IF NOT EXISTS pict_placement_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
    ...
    connection = await mysql.createConnection({
      ...
      user: process.env.DB_USER || 'root',
      password: process.env.DB_PASSWORD || 'Raj27',
      database: 'pict_placement_db'
    });

    console.log('📊 Connected to pict_placement_db database');
    ...
    await connection.execute('CREATE INDEX idx_students_cgpa ON students(current_cgpa)');
    await connection.execute('CREATE INDEX idx_students_branch_class ON students(branch, class)');
    await connection.execute('CREATE INDEX idx_jobs_package ON job_postings(package_offered)');
    await connection.execute('CREATE INDEX idx_applications_student_status ON applications(student_id, status)');
